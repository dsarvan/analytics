...
#C = (1/len(nprice)) * (nprice.T @ nprice)
#print(C)

#gprime = (nprice)/np.sqrt(len(nprice))
#C1 = (gprime.T @ gprime)
#print(C1)

#A = np.random.normal(loc = 0, scale = 1, size = nprice.shape)
#A = np.random.randn(size = nprice.shape)
#print(A)
#print(A.shape)

A = pd.DataFrame(np.random.normal(loc = 0, scale = 1, size = nprice.shape))
