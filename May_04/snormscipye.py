...
# compute cumulative distribution function (CDF): P(-1.85 < Z < -0.90)
x = -1.85; y = -0.90; loc = 0; scale = 1
pls = norm.cdf(y, loc, scale) - norm.cdf(x, loc, scale)
print("Area under the standard normal curve P(-1.85 < Z < -0.90): {:.5f}".format(pls))
...
ax.plot(x2,y2,c='r',label='P(-1.85 < Z < -0.90) = {:.5f}'.format(pls))
