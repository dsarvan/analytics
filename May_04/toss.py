...
option1 = input("Type L for at least, M for at most, E for extact: ")
option2 = input("Type H for Heads, T for Tails: ")

if option2 == 'H':
    side = 'Heads'
    print("[Note: Assigned value 1 for Head and 0 for Tail]")
else:
    side = 'Tails'
    print("[Note: Assigned value 0 for Head and 1 for Tail]")

data = []
numTrails = 10_00_000

for m in range(numTrails):
    flips = [randint(0,1) for n in range(coin)]
...
data = np.array([data[i:i+coin] for i in range(0, len(data), coin)]).tolist()
...
if option1 == 'L':
    ...
    evalue = int(input("Exact number of {}: ".format(side)))
