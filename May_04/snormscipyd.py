...
# compute cumulative distribution function (CDF): P(Z < -1.88)
x = -1.88; loc = 0; scale = 1; pls = norm.cdf(x, loc, scale)
print("Area under the standard normal curve P(Z < -1.88): {:.5f}".format(pls))
...
ax.plot(x2,y2,c='r',label='P(Z < -1.88) = {:.5f}'.format(pls))
